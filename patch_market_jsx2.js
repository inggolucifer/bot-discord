...
// Find the end of the return statement where Modals usually are, or just before the final `</div>`

// It looks like my previous replace `content.replace('<Modal isOpen={buyModalOpen}', ...)` failed because `buyModalOpen` modal string might have been modified or spaced differently.
// Let's replace the last `</div>` with our modal code.

const lastDivIndex = content.lastIndexOf('</div>\n  );\n}');
if (lastDivIndex !== -1) {
    content = content.slice(0, lastDivIndex) + sellModalJSX + '\n    </div>\n  );\n}';
    fs.writeFileSync(filePath, content);
    console.log('Injected sell modal JSX correctly.');
} else {
