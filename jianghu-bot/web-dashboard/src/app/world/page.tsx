                          ...
                          <div className="grid md:grid-cols-2 gap-4">
                              {locationData.npcsHere.map((npc: any) => (
                                  <div key={npc._id} className="bg-[#1e2532] border border-[#2a3142] rounded-lg p-4 flex justify-between items-center hover:border-blue-500/50 transition-colors">
                                      <div>
                                          <h3 className="text-lg font-bold text-gray-200">{npc.imageUrl ? <FallbackImage src={npc.imageUrl} alt={npc.name} className="w-8 h-8 rounded-full inline-block mr-2" fallbackNode={<span className="mr-2">👤</span>} /> : <span className="mr-2">👤</span>} {npc.name}</h3>
