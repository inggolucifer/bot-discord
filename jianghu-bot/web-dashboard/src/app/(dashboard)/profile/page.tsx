...
export default function ProfilePage() {
  ...
  const equipableItems = profile.inventory?.filter((inv: any) => {
    ...
    const isEq = inv.isEquipped === true;
    const catMatch = ['weapon', 'armor', 'cloth', 'helmet', 'pants', 'boots', 'accessories', 'accessory', 'mount'].includes(inv.itemId?.category);
    const notInSlots = !Object.values(equipment).some(equippedId => equippedId && String(inv._id) === String(equippedId));
    return !isEq && catMatch && notInSlots;
  }) || [];
    ...
    <div className="p-3 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 space-y-6 flex flex-col">
          ...
          <div className="bg-[#1a1a1a] border border-[#c5a880]/30 rounded-lg p-6 shadow-lg w-full z-10 relative mt-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full items-start">
             <div className="order-1 lg:order-none lg:col-start-1 flex flex-col items-center w-full">
             ...
             {/* Inventory to Equip */}
             <div className="order-2 lg:order-none lg:col-start-2 lg:row-span-2 flex flex-col h-full bg-black/20 rounded-lg border border-[#333] p-4 min-h-[300px] lg:min-h-[500px]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3 pb-4 mt-2 max-h-[600px]">
                   {equipableItems.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-gray-500 text-sm text-center p-6 border border-dashed border-[#444] rounded bg-black/40 mt-4">
                         <div className="text-4xl mb-3 opacity-30">🎒</div>
                         <p className="mb-2">Tidak ada equipment yang bisa dipakai.</p>
                         <a href="/inventory" className="text-[#c5a880] text-xs hover:underline flex items-center gap-1">Buka Inventory ➔</a>
                      </div>
                   ) : (
            ...
            <div className="w-full bg-black/40 p-4 rounded border border-[#333] order-3 lg:order-none lg:col-start-1">
