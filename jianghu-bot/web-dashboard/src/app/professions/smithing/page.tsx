...
export default function SmithingPage() {
    ...
    const { data: recipesRaw, isLoading: isLoadingRecipes } = useQuery({
        queryKey: ['recipes', 'smithing'],
        queryFn: () => api.get('/professions/recipes?profession=smithing').then(res => res.data),
    });
    ...
    const smithingProf = profile.professions?.smithing;
    if (!smithingProf?.isUnlocked) {
        return (
            <div className="container mx-auto p-4 max-w-4xl text-center mt-20">
                ...
                <p className="text-gray-400 mb-6">Kamu belum mempelajari profesi Menempa.</p>
                <button onClick={() => router.push('/professions')} className="px-6 py-2 bg-gray-600 rounded text-white font-bold">
    ...
    const handleStart = () => {
        ...
        startMutation.mutate({ profession: 'smithing', recipeId, toolItemId });
    };
    ...
    return (
        <div className="container mx-auto p-4 max-w-4xl text-gray-100">
            <div className="flex justify-between items-center mb-4">
                <button onClick={() => router.push('/professions')} className="text-gray-400 hover:text-gray-300 font-bold flex items-center gap-2">
                    &larr; Kembali
                </button>
                <ProfessionSkillBadge level={smithingProf.level || 1} exp={smithingProf.exp || 0} />
            </div>

            <PageHeader title="Menempa" description="Tempa logam jadi senjata dan alat kuat." />

            {!activeSessionId ? (
                <div className="bg-gray-900 border border-gray-500/50 p-8 rounded-xl shadow-2xl mt-8">
                    <h2 className="text-2xl font-bold mb-6 text-gray-300 border-b border-gray-700 pb-2">Persiapan Menempa</h2>
                    ...
                    <div className="flex justify-end gap-4 border-t border-gray-800 pt-6">
                        <button
                            ...
                            className="px-8 py-3 rounded-lg bg-gray-600 hover:bg-gray-500 text-white font-bold text-lg shadow-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {startMutation.isPending ? 'Memulai...' : 'Mulai Menempa (-10 Energy)'}
                        </button>
                    </div>
                </div>
            ) : (
                <div className="mt-8 bg-gray-900 border border-gray-500/50 rounded-xl shadow-2xl overflow-hidden">
                    <SmithingMinigame
