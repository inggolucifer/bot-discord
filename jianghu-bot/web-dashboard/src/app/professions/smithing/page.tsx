        ...
        <div className="container mx-auto p-4 max-w-4xl text-gray-100">
            ...
            {!activeSessionId ? (
                <div className="bg-gray-900 border border-amber-500/50 p-8 rounded-xl shadow-2xl mt-8">
                    <h2 className="text-2xl font-bold mb-4 text-amber-500">Persiapan Menempa</h2>
                    <p className="mb-4 text-gray-300">Pilih resep dan palu tempa (Simulasi).</p>

                    <div className="space-y-4 mb-6">
                        <div>
                            <label className="block text-sm text-gray-400 mb-1">Nama Resep</label>
                            <input type="text" value={recipeId} onChange={e => setRecipeId(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded p-2 text-white" placeholder="Pedang Besi" />
                        </div>
                        <div>
                            <label className="block text-sm text-gray-400 mb-1">Tool Item ID</label>
                            <input type="text" value={toolItemId} onChange={e => setToolItemId(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded p-2 text-white" placeholder="ObjectId Palu" />
                        </div>
                    </div>

                    <div className="flex justify-end gap-4">
                        <button onClick={() => startMutation.mutate({ profession: 'smithing', recipeId, toolItemId })} disabled={startMutation.isPending} className="px-6 py-2 rounded bg-amber-600 hover:bg-amber-500 text-black font-bold transition">
