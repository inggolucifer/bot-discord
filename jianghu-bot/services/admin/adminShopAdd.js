...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let listing = await Shop.findOne({ category: kategori, refId: doc._id });
    if (listing) {
      listing.price = harga; listing.priceCurrency = currency; listing.stock = stok; listing.isActive = true;
    } else {
      listing = new Shop({
        category: kategori, refId: doc._id, refModel: REF_MODEL_MAP[kategori],
        price: harga, priceCurrency: currency, stock: stok, addedBy: interaction.user.id,
      });
    }
    await listing.save();

    await logAdminAction(interaction.client, { adminId: interaction.user.id, action: 'SHOP_ADD', details: `${doc.name} (${kategori}) - ${harga} ${currency}, stok: ${stok === -1 ? 'unlimited' : stok}` });
